...
class Rule extends ObjectBase{
	...
	{	
		...
		$id_rule = Db::getInstance()->getValue('SELECT id_rule FROM `'._DB_PREFIX_.'rule` '.$where);
	...
	public static function getRule($active = true,$p=1,$limit=50,$orderBy = NULL,$orderWay = NULL,$filter=array())
	{
		...
		$total  = Db::getInstance()->getRow('SELECT count(*) AS total FROM `'._DB_PREFIX_.'rule` a
				WHERE 1 '.$where);

		$result = Db::getInstance()->ExecuteS('SELECT * FROM `'._DB_PREFIX_.'rule` a
				WHERE 1 '.$where.'
				'.$postion.'
	...
	{
		return Db::getInstance()->getValue('SELECT `id_rule` FROM `'._DB_PREFIX_.'rule` WHERE `rule_link`="'.pSQL($r).'"');
	}
	
	public static function deleteByUrlLink($link)
	{
		return Db::getInstance()->Execute('DELETE FROM `'._DB_PREFIX_.'rule` WHERE `rule_link`="'.pSQL($link).'"');
	}
}
