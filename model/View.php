...
class View{
	...
	{
		if(isset($_GET['rule']))
			$args = explode('/',$_GET['rule']);
		else
			$args = array('index.html');
		return $args;
	} 
	...
	public static function run()
	{
		...
		if($result && file_exists(_TM_VIEWS_DIR.$result['view_name'].'.php')){
			...
			$entity->id	= $result['id_entity'];
			$entity->load();
			$view         = new $result['view_name'];
			$view->entity = $entity;
			$_GET['entity'] 	= $result['entity'];
			$_GET['id_entity'] 	= $result['id_entity'];
		}else{
	...
	{	
		$this->displayHeader();
		if(method_exists($this,'displayLeft'))
			$this->displayLeft();
		$this->displayMain();
		if(method_exists($this,'displayRight'))
			$this->displayRight();
		$this->displayFooter();
	}
	...
	public function pagination($nbProducts = 10)
	{
		...
		if($arg_string = http_build_query($arg))
			$this_link =  Tools::getLink($this->entity->rewrite).'?'.$arg_string;
		else
			$this_link = Tools::getLink($this->entity->rewrite);
