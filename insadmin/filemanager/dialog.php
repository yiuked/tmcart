...
//view type
	...
	<head>
		...
		<script type="text/javascript" src="<?php echo _TM_JQ_URL;?>jquery.min.js"></script>
		...
		<script type="text/javascript" src="js/include.min.js.js"></script>
	</head>
	<body>
					...
					<div class="filters">
						<div class="row-fluid">
							...
							<div class="span6 types">
								...
								<label id="ff-item-type-all" title="<?php echo Tools::safeOutput(lang_All); ?>" <?php if (Tools::Q('type') == 1 || Tools::Q('type') == 3){ ?>style="visibility: hidden;" <?php } ?> data-item="ff-item-type-all" for="select-type-all" style="margin-rigth:0px;" class="tip btn btn-inverse ff-label-type-all"><i class="icon-align-justify icon-white"></i></label>
	...
	if (in_array($extension_lower, $ext_img))
	{
		...
		if (strpos($src_thumb, _TM_ROOT_DIR_) === 0)
			$mini = $thumb = __TM_BASE_URI__.substr($thumbs_path.$subdir.$file, strlen(_TM_ROOT_DIR_) + 1);
		//add in thumbs folder if not exist
	...
	if ((!(Tools::Q('type') == 1 && !$is_img) && !((Tools::Q('type') == 3 && !$is_video) && (Tools::Q('type') == 3 && !$is_audio))) && $class_ext > 0){
	?>
